...
# ---------------------------------------------------------------------------
# Testes do solver (test_solver.py) — mínimo 6 testes
# ---------------------------------------------------------------------------


class TestSolverOtimizacaoBasica:
    """Testes das propriedades da solução ótima. Cobre: AC-2."""
    ...
    def test_instancia_classica_do_enunciado(self):
        ...
        Cobre AC-2: status OPTIMAL, barras > 0, desperdício >= 0, plano não vazio.
        """
        itens = [
...
class TestSolverValidacaoSchemas:
    """Testes das validações Pydantic do schema de entrada. Cobre: AC-3, AC-4, AC-5."""

    def test_comprimento_padrao_zero_invalido(self):
        """comprimento_padrao=0 deve levantar ValidationError. Cobre: AC-3."""
    ...
    def test_comprimento_padrao_negativo_invalido(self):
        """comprimento_padrao negativo deve levantar ValidationError. Cobre: AC-3."""
        from pydantic import ValidationError
    ...
    def test_item_comprimento_maior_que_barra_invalido(self):
        """Item com comprimento > comprimento_padrao deve ser rejeitado. Cobre: AC-4."""
        from pydantic import ValidationError
    ...
    def test_quantidade_zero_invalida(self):
        """quantidade=0 deve levantar ValidationError. Cobre: AC-5."""
        from pydantic import ValidationError
    ...
    def test_quantidade_negativa_invalida(self):
        """quantidade negativa deve levantar ValidationError. Cobre: AC-5."""
        from pydantic import ValidationError
    ...
    def test_time_limit_acima_maximo_invalido(self):
        """time_limit > 120 deve levantar ValidationError. Cobre: AC-10."""
        from pydantic import ValidationError
    ...
    def test_time_limit_maximo_valido(self):
        """time_limit=120 é o limite superior e deve ser aceito. Cobre: AC-9."""
